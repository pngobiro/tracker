...
class SmsControllerTest < ActionController::TestCase
  ...
  test "should create sm" do
    assert_difference('Sm.count') do
      post :create, sm: { number: @sm.number, sentmessage: @sm.sentmessage, station: @sm.station, status: @sm.status }
    end
  ...
  test "should update sm" do
    put :update, id: @sm, sm: { number: @sm.number, sentmessage: @sm.sentmessage, station: @sm.station, status: @sm.status }
    assert_redirected_to sm_path(assigns(:sm))
  end
